...
public class SFCDBankManager {
    ...
    public MapLayout importSFCDBank(MapBlock[] blockset, byte[] layoutData){
        ...
        try{
            ...
            if(this.blockset!=null){
                layout = parseLayoutData(this.blockset, layoutData);
            }
        }catch(Exception e){
    ...
    private MapLayout parseLayoutData(MapBlock[] blockSet, byte[] layoutData){
        MapLayout layout = new MapLayout();
    ...
    public void exportDisassembly(MapBlock[] blockset, MapLayout layout, String layoutFilePath){
        System.out.println("com.sfc.sf2.maplayout.io.DisassemblyManager.exportDisassembly() - Exporting disassembly ...");
        try { 
            byte[] layoutBytes = produceLayoutBytes(blockset, layout);
    ...
    private byte[] produceLayoutBytes(MapBlock[] origblocks, MapLayout layout){
        
        blockset = produceNewBlockset(origblocks, layout);
