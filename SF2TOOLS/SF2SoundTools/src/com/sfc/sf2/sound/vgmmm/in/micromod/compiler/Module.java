...
public class Module implements Element {
	private com.sfc.sf2.sound.vgmmm.in.micromod.Module module;
	private com.sfc.sf2.sound.vgmmm.in.micromod.Macro[] macros = new com.sfc.sf2.sound.vgmmm.in.micromod.Macro[ 100 ];
	...
	public void begin( String value ) {
		module = new com.sfc.sf2.sound.vgmmm.in.micromod.Module();
	...
	public void end() {
		...
		com.sfc.sf2.sound.vgmmm.in.micromod.Pattern[] patterns = new com.sfc.sf2.sound.vgmmm.in.micromod.Pattern[ module.getSequenceLength() ];
		for( int sequenceIdx = 0; sequenceIdx < patterns.length; sequenceIdx++ ) {
			com.sfc.sf2.sound.vgmmm.in.micromod.Pattern pattern = module.getPattern( module.getSequenceEntry( sequenceIdx ) );
			patterns[ sequenceIdx ] = new com.sfc.sf2.sound.vgmmm.in.micromod.Pattern( numChannels, pattern );
		}
		for( int patternIdx = 0; patternIdx < patterns.length; patternIdx++ ) {
			com.sfc.sf2.sound.vgmmm.in.micromod.Pattern pattern = patterns[ patternIdx ];
			for( int rowIdx = 0; rowIdx < com.sfc.sf2.sound.vgmmm.in.micromod.Pattern.NUM_ROWS; rowIdx++ ) {
				...
				for( int channelIdx = 0; channelIdx < numChannels; channelIdx++ ) {
					pattern.getNote( rowIdx, channelIdx, note );
					com.sfc.sf2.sound.vgmmm.in.micromod.Macro macro = macros[ note.instrument ];
					if( macro != null ) {
						note.instrument = 0;
	...
	public com.sfc.sf2.sound.vgmmm.in.micromod.Instrument getInstrument( int insIdx ) {
		return module.getInstrument( insIdx );
	}
	
	public void setMacro( int macroIdx, com.sfc.sf2.sound.vgmmm.in.micromod.Macro macro ) {
		if( macroIdx > 0 && macroIdx < 100 ) macros[ macroIdx ] = macro;
	}
	
	public com.sfc.sf2.sound.vgmmm.in.micromod.Pattern getPattern( int patIdx ) {
		return module.getPattern( patIdx );
	}
	
	public com.sfc.sf2.sound.vgmmm.in.micromod.Module getModule() {
		return module;
	}
